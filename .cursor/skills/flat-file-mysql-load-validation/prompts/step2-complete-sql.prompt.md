---
title: "CSVからインポートし重複削除まで行うSQL生成（DB名指定版）"
description: "CSVサンプルを基に、一時テーブルへのデータロード、重複排除、最終テーブル作成までの一連のSQLを生成します。データベース名は指定可能。"
mode: agent
---

## 概要

このプロンプトは、CSVファイルからデータをデータベースに取り込み、重複を除いたクリーンなテーブルを作成するまでの一連のSQL文を自動生成するためのものです。**データベース名は `{{database_name}}` で指定する。** エージェントはユーザーから受け取った DB 名をここに渡すこと。未指定の場合は「DB名を明示してから実行してください」と表示してストップする。
...
4. **DB 名**: エージェントがユーザーに問い合わせた `{{database_name}}` を使用してください。

### 要件
1. **データベース名**: `{{database_name}}` を使用してください。（エージェントがユーザーから取得した値で置換する）
2. **テーブル名**:
...
9. **出力形式**:
    ...
    - **データベース名**: 生成する SQL では `USE {{database_name}};` またはテーブル参照を `{{database_name}}.[テーブル名]` の形で使用すること。
