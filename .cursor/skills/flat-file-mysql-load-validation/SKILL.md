...
- 実行 cwd は **プロジェクトルート**。
- ステップ 2 の成果物は `./skill-output/step2_complete_sql` に保存する。
- ステップ 3 のレポートは `./skill-output/step3_report` に保存する。
...
3. 完成版 SQL を `./skill-output/step2_complete_sql` に保存する。複数 CSV の場合は CSV ごとに 1 本。
...
1. 上記の問い合わせでユーザーが実施すると答えた場合のみ、エージェントが **Python CLI**（ステップ 3 用：SQL 実行・件数比較）を呼び出す。CLI に完成版 SQL のパスと対象 DB 接続情報を渡す。
   - `python3 .cursor/skills/flat-file-mysql-load-validation/scripts/step3_cli.py <complete.sql> -d <database> --table <table> --expected-count <n> --report-dir ./skill-output/step3_report`
