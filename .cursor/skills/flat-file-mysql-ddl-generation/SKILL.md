...
- 実行 cwd は **プロジェクトルート**。
- 出力先は `./skill-output/step1_sample_sql`（`-o/--out-dir` 未指定時の既定）。
...
- CSV パスが存在することを確認する。
- 実行前に `./skill-output/step1_sample_sql` へ出力されることをユーザーに明示する。
...
2. **CLI 呼び出し**: 次のコマンドでステップ 1 用 CLI を実行する。
   - `python3 .cursor/skills/flat-file-mysql-ddl-generation/scripts/step1_cli.py <csv1> [csv2 ...] -o ./skill-output/step1_sample_sql`
3. **成果物の確認**:
