...
- **入力データ**: DB から抽出した分析データセットを使う場合は、可能な限り `sql/validated/` 配下の SQL と `query_note.md` を参照し、抽出条件と粒度を確認する。
...
- レポート本文は原則 `./skill_out/vcd_categorical/vcd_analysis_report.md` に保存する。
- 参照した SQL、抽出条件、データセット粒度はレポート冒頭または注記に残す。
