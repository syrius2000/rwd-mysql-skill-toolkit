...
description: "【必須ワークフロー】必ず以下の3ステップを順に実行すること: 1. analysis.Rによるデータ集約 2. AIレビューの作成とMDファイルへの保存 3. dashboard.Rmdのレンダリング。途中でユーザーに回答を返さず、全ステップを自動で完遂すること。"
license: MIT
metadata:
  version: "1.0"
---
...
## ⚠️ MANDATORY WORKFLOW (AIへの厳格な指示)
このスキルを呼び出した場合、あなたは**必ず以下の3ステップを連続して実行**しなければなりません。途中でチャットに結果を要約して終了してはいけません。

1. **Step 1 (Data Aggregation)**: `analysis.R` を `run_shell_command` ツールで実行し、`evidence_results.json` が生成されたことを確認する。
2. **Step 2 (AI Review Generation)**: JSONの中身を読み込み、指定されたプロンプトに従って日本語の考察を作成し、**必ず `write_file` ツールを使って `executive_summary.md` として保存する**（チャット上に直接出力しないこと）。
3. **Step 3 (Report Integration)**: `dashboard.Rmd` を `run_shell_command` ツールでレンダリングし、HTMLファイルが生成されたことを確認してから、ユーザーに完了を報告する。
