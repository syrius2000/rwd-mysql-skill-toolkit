...
├── code_understanding_report.md  # Full
├── code_review_report.md         # Review
├── code_documentation.md         # Documentation
├── refactoring_proposal.md       # Refactoring
└── run_meta.json
```

実際にはモードに対応するMarkdownファイルを1つだけ生成する。`run_meta.json` には `skill`、`skill_version`、`mode`、`target`、`report_file`、`generated_at` を記録する。
...
出力先を自動生成するため、`--run-id` を省略してもよい。CLIは保存したMarkdownと `run_meta.json` のパスを標準出力する。
