-- Anomaly detection input for VACCINE multi-table consistency test.
-- Grain: one row per CH_t05_covid_vaccine record enriched with demographics,
-- covid-test counts, and outcome counts.
...
WITH test_summary AS (
  SELECT
    ...
    MIN(COLLECTDATE) AS first_test_date,
    MAX(COLLECTDATE) AS last_test_date
...
outcome_summary AS (
  SELECT
    ...
    MIN(ADMISSIONDATE) AS first_admission_date,
    MAX(ADMISSIONDATE) AS last_admission_date,
